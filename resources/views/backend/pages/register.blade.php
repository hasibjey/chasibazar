                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 col-4">
                            <div class="card card-info">
                                <div class="card-header">
                                    <h3 class="card-title text-capitalize">{{ empty($update)? 'add' : 'update' }} admin information</h3>
                                </div>
                                <form action="{{ empty($update)? route('admin.setting.admin.store') : route('admin.setting.admin.update') }}" method="post">
                                    @csrf
                                    <input type="text" name="id" value="{{ $update->id ?? null}}" hidden>
                                    <div class="card-body">
                                        <div class="form-group">
                                            <label>Full name</label>
                                            <input type="text" class="form-control" placeholder="Enter full name" name="name" value="{{ $update->name ?? old('name')}}">
                                            @error('name')
                                                    ...
                                                    @foreach ($roles as $role)
                                                        <div class="col-6 py-1">
                                                            <div class="form-check">
                                                                <input class="form-check-input" type="radio" name="role" value="{{ $role }}" {{ isset($update) && $update->roles->pluck('name')->contains($role)? 'checked' : null}}>
                                                                <label class="form-check-label text-capitalize">{{ $role }}</label>
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                @endif
